...
export const apiUpdateCV = (
  id: string,
  data: ICVDataModel,
): Promise<AxiosResponse<ICVDataModel, any>> => {
...
export const apiGetCVById = (id: string): Promise<AxiosResponse<ICVDataModel, any>> => {
  return api.get<ICVDataModel>(`/cv-data/${id}`);
};

export const apiCvView = (id: string, key?: boolean): Promise<AxiosResponse<ICVDataModel, any>> => {
  return api.get<ICVDataModel>(`/cv-data/view/${id}`, {
    isKey: key,
  });
};
...
export const uploadPhoto = (
  ...
  isEdit: boolean,
  cvId: string,
): Promise<AxiosResponse<ImageStatus, any>> => {
  const formData = new FormData();
  formData.append('file', image as Blob);

  return api.post<ImageStatus>(
    `/cv-data/photo?isEdit=${isEdit}&cvId=${cvId === '' ? 'empty' : cvId}`,
    formData,
    {
      headers: {
...
export const removePhoto = (
  ...
  isEdit: boolean,
  cvId: string,
): Promise<AxiosResponse<ImageRemovedModel, any>> => {
...
export const apiDownloadPdf = (url: string): Promise<Blob> => {
  return api
    .post(`/pdf/generate?url=${url}`, null, {
      responseType: 'blob',
    })
    .then((res) => res.data);
};
